...
Export only governance entities created via GraphQL (excludes ingestion-created):

```bash
python -m src.cli.export_cmd --output-dir metadata/ --filter-by-source ui
```

> **Caveat**: DataHub labels **all** GraphQL mutations as `appSource: "ui"` — not just browser interactions. Scripts and SDK calls that use the `/api/graphql` endpoint receive the same label. The `--filter-by-source ui` flag therefore means "via GraphQL" in practice, excluding only entities written through the REST/OpenAPI ingest path (e.g., `datahub ingest`, `DataHubGraph.emit_mcp()`).
...
| **Aspect provenance is last-writer-wins** | DataHub records provenance per aspect via `systemMetadata`: GraphQL mutations set `appSource: "ui"`, ingestion sets `runId` + `pipelineName`, and this pipeline tags writes with `appSource: "cicd-pipeline"`. However, only the **most recent writer** is recorded — there is no audit history. If an ingestion pipeline overwrites a UI-authored aspect, the UI provenance is lost (see [Scenario 6 in conflict-behavior.md](docs/conflict-behavior.md#scenario-6-concurrent-edits-in-prod-overwrite)). Entities created before systemMetadata tracking have no provenance at all. **Note:** `appSource: "ui"` is set by all GraphQL mutations, not just browser interactions — scripts using the GraphQL API receive the same label. | Provenance filtering via `--filter-by-source ui` exports GraphQL-authored and CI/CD-authored governance entities (excluding REST/OpenAPI ingestion). Entities with unknown provenance are included to avoid false exclusions. Enrichment is additionally filtered by governance URN references regardless of provenance. |
